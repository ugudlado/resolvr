...
2. Parse `committedDiff` using a lightweight diff header parser (extract file paths and statuses -- does not need full hunk parsing for the navigator)
...
3.   serverClient.getDiff(workspaceRoot)
       ...
       <- { committedDiff, sourceBranch, targetBranch, ... }
4.   Parse committedDiff headers to extract file list
...
### D4: Committed diff only vs. including uncommitted changes

**Decision**: Committed diff only (`main...HEAD`).

**Rationale**: The diff panel is for reviewing changes that are ready for merge -- committed, intentional changes. Uncommitted changes are in flux and better served by the existing inline working-tree comments and VS Code's built-in SCM diff. Including uncommitted changes would complicate the base content provider (need `HEAD:<path>` for committed view vs. `main:<path>` for full diff) and create confusion about what is being reviewed.

[ASSUMPTION: Users commit their changes before self-reviewing. If uncommitted diff support is needed, it can be added later by changing the `ref` parameter on the base content provider.]
