...
A dedicated diff panel that shows `main...HEAD` diffs with inline comment support would close all three gaps. The developer sees exactly what changed, can comment on both old and new sides, and can navigate files systematically -- all without leaving VS Code.

## Requirements

### R1: Open diff panel for feature branch

A command "Local Review: Open Diff" opens a multi-file diff view showing all changes between `main` and `HEAD` (i.e., `git diff main...HEAD`). The diff data comes from the server's existing `GET /api/context/diff` endpoint. If no feature branch is detected or no server connection exists, the command shows an appropriate error message.

### R2: Native VS Code diff editor per file

Each changed file opens as a VS Code diff editor tab (side-by-side or inline, respecting the user's `diffEditor.renderSideBySide` setting). The left side shows the file content at the merge-base (`main`), the right side shows the current working-tree content. The extension uses a `TextDocumentContentProvider` with a custom URI scheme (e.g., `local-review-base:`) to serve the old-side file content via `git show main:<path>`.
...
- **Uncommitted changes in diff**: The diff panel shows `main...HEAD` (committed changes only). Uncommitted working-tree changes are not included. The existing inline comments cover working-tree files.
...
2. Clicking a file opens a native VS Code diff editor with the correct old (main) and new (HEAD) content.
