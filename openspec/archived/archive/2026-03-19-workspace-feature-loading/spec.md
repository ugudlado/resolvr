...
The git state cache MUST be keyed by workspace path, not stored as a single module-level variable. Concurrent requests for different workspaces MUST NOT corrupt each other's state. ALL git-state consumers (`features.ts`, `context.ts`, `spec.ts`, `utils.ts`, and any other route/helper that calls `getGitState()`) MUST be updated to pass the resolved `repoRoot`.

**Acceptance:** `GET /api/features?workspace=review` followed by `GET /api/features?workspace=lens` returns correct features for each, with no cross-contamination. Other endpoints (`/api/context`, `/api/spec`, etc.) also return correct workspace-scoped data.

### R4 — Active Workspace Tracking

The workspace registry MUST track which workspace was last active. `lastActive` MUST be stored as the workspace's absolute path (not name), since workspace names are basenames and may collide across repos with the same directory name.

Registration (`registerWorkspace`) MUST set `lastActive` only when called from explicit user actions (API registration endpoint, session-start hook). Server startup's self-registration of the plugin repo MUST NOT overwrite `lastActive`.

**Acceptance:** `~/.config/local-review/workspaces.json` includes a `lastActive` field (absolute path) that updates on explicit registration and feature queries, but not on server startup self-registration.
...
### R8 — Stale Workspace Handling

When a workspace path in the registry no longer exists on disk, the server MUST NOT crash. The workspace MUST be preserved in the registry (not pruned) but marked unavailable. Feature queries for unavailable workspaces MUST return an empty feature list with a clear indicator, not a 500 error.

**Acceptance:** Deleting a workspace directory from disk does not crash the server; re-creating the directory restores full functionality without re-registration.
...
| Workspace deleted from disk but still in registry | Preserved in registry; returns empty feature list, does not crash                 |
...
| Two repos with same basename (e.g. both `app`)    | Both registered; `lastActive` uses path (not name) so no ambiguity                |
| Server restarts after user switches workspace     | `lastActive` persists; dashboard loads the user's last workspace, not plugin repo |

## Review Summary

**[codex]** Three critical issues identified and resolved:

1. `lastActive` changed from name-based to path-based to avoid basename collisions
2. Git state cache scope expanded from `features.ts`-only to ALL git-state consumers
3. Startup self-registration separated from `lastActive` mutation to prevent overwrite on restart

**[codex]** Two suggestions incorporated:

1. R8 added for explicit stale workspace handling (preserve, don't prune)
2. Atomic write requirement noted in design (current `writeFileSync` needs upgrade)
